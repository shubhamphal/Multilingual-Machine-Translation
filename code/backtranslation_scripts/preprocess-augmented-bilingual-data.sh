...
RAW_DDIR=data/ted_raw/aze_eng_augmented/backtranslation/
...
for i in ${!LANGS[*]}; do
  ...
  TRAIN_FILES="$RAW_DDIR"ted-train.orig."$LANG","$RAW_DDIR"ted-train.orig.eng
  ...
  python "$SPM_ENCODE" \
	  --model=$spm_model \
	  --output_format=piece \
	  --inputs "$RAW_DDIR"ted-train.orig."$LANG" "$RAW_DDIR"ted-train.orig.eng  \
  ...
  do
    python "$SPM_ENCODE" \
	    --model=$spm_model \
	    --output_format=piece \
	    --inputs "$RAW_DDIR"ted-"$split".orig."$LANG" "$RAW_DDIR"ted-"$split".orig.eng  \
